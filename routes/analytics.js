...
router.get('/', verifyToken, async (req, res) => {
  try {
    ...
    // TOTAL HOTEL ROOMS
    // ─────────────────────────────────────
    const totalHotelRooms = hotel.rooms.reduce(
    ...
    // ─────────────────────────────────────
    const uniqueGuests =
      Object.keys(phoneMap).length;

    const repeatGuestsCount =
      Object.values(phoneMap).filter(
        (count) => count > 1
      ).length;
    ...
    // TOP GUESTS
    // ─────────────────────────────────────
    const finalTopGuests =
      Object.values(guestMap)
        .sort(
          (a, b) =>
            b.spentNum - a.spentNum
        )
        .slice(0, 4)
        .map((guest) => ({
          ...guest,
          spent: `₹${guest.spentNum.toLocaleString()}`,
        }));
    ...
    res.status(500).json({
      message:
        'Server error fetching analytics data',
    });
  }
});
